...
const REPORT_TITLES: Record<string, string> = {
  ...
  commissions: "COMMISSIONS & STAFF PERFORMANCE REPORT",
...
export default async function PrintableReportPage({
  ...
  const type = REPORT_TITLES[searchParams.type ?? ""] ? (searchParams.type as string) : "summary";
    ...
    <div className="min-h-screen bg-background py-10 px-4">
      ...
      <div className="print-receipt max-w-4xl mx-auto bg-surface-container-lowest rounded-xl shadow-level-1 p-card-padding">
        <div className="text-center mb-8 border-b border-outline-variant pb-6">
          ...
          <p className="text-label-caps font-label-caps text-on-surface-variant mt-3">{REPORT_TITLES[type]}</p>
          <p className="text-body-md text-on-surface mt-1">{periodLabel}</p>
        </div>

        {type === "summary" && <SummarySection from={from} to={to} currency={currency} />}
        {type === "expenses" && <ExpensesSection from={from} to={to} currency={currency} />}
        {type === "commissions" && <CommissionsSection from={from} to={to} currency={currency} />}
        {type === "washes" && <WashesSection from={from} to={to} currency={currency} />}

        <p className="text-center text-xs text-on-surface-variant mt-10">
          Generated {new Date().toLocaleString("en-GB")} — {settings.businessName}
        </p>
      </div>
    </div>
  );
...
async function SummarySection({ from, to, currency }: { from: Date; to: Date; currency: string }) {
...
async function ExpensesSection({ from, to, currency }: { from: Date; to: Date; currency: string }) {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-headline-md font-headline-md">Expenses</h2>
        <span className="text-sm text-on-surface-variant">
          Total: <strong className="text-on-surface">{formatMoney(total, currency)}</strong>
        </span>
      </div>
      {expenses.length === 0 ? (
        <p className="text-sm text-on-surface-variant">No expenses recorded in this period.</p>
      ) : (
        <table className="w-full text-sm border-collapse">
          ...
          <tfoot>
            <tr className="bg-surface-container-low font-medium">
              ...
              <td className="py-2 px-3 text-right">{formatMoney(total, currency)}</td>
            </tr>
          </tfoot>
        </table>
      )}
...
async function CommissionsSection({ from, to, currency }: { from: Date; to: Date; currency: string }) {
  const [staffRows, payouts] = await Promise.all([
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-headline-md font-headline-md">Commission Payouts</h2>
        <span className="text-sm text-on-surface-variant">
          Total paid out: <strong className="text-on-surface">{formatMoney(totalCommissionPaid, currency)}</strong>
        </span>
      </div>
      {payouts.length === 0 ? (
        <p className="text-sm text-on-surface-variant">No commission payouts recorded in this period.</p>
      ) : (
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-surface-container-low text-left">
          ...
          <tbody>
            {payouts.map((p) => (
              <tr key={p.id} className="border-b border-outline-variant/30 align-top">
                <td className="py-2 px-3">{(p.paidAt ?? p.createdAt).toLocaleDateString("en-GB")}</td>
...
async function WashesSection({ from, to, currency }: { from: Date; to: Date; currency: string }) {
  const washes = await prisma.washRecord.findMany({
    where: { createdAt: { gte: from, lte: to } },
    include: { staff: true },
    orderBy: { createdAt: "asc" },
  });
  const total = washes.reduce((sum, w) => sum + Number(w.totalAmount), 0);

  return (
    <>
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-headline-md font-headline-md">Wash Records ({washes.length})</h2>
        <span className="text-sm text-on-surface-variant">
          Total: <strong className="text-on-surface">{formatMoney(total, currency)}</strong>
        </span>
      </div>
          ...
          <tbody>
            {washes.map((w) => (
              <tr key={w.id} className="border-b border-outline-variant/30">
                <td className="py-2 px-3">{w.createdAt.toLocaleDateString("en-GB")}</td>
